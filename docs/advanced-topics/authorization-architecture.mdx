...
# User ID header delegation (optional - proxy scenario)
Parameters__UserIdHeaderName=X-User-Id
Parameters__UserIdHeaderRequired=false
...
When the API service runs behind an authenticating proxy that uses a managed identity to forward requests, the delegated user ID from an HTTP header can be used instead of the managed identity's sub claim. This is configured via:
...
When enabled, the `UserIdHeaderMiddleware` reads the header value (e.g., a B2C user ID injected by the proxy) and creates a synthetic `ClaimsIdentity` prepended to the `ClaimsPrincipal`. All downstream authorization checks and `TrackLastUpdatedBy` use this synthetic identity.

For the `Api` permission provider, the synthetic identity's `sub` claim (from the header) is used for permission lookups. For the `Claims` provider, original managed identity permission claims are preserved and accessible.
