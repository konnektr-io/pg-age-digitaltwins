...
All settings can be customized through the `Parameters` section in `appsettings.json`. See the [Configuration Reference](/docs/graph/reference/configuration) for the complete list of parameters, including defaults.

Key defaults:

| Setting | Default |
|---------|---------|
| `Parameters:GlobalPermitLimit` | `1000` requests/second |
| `Parameters:LightOperationsPermitLimit` | `100` requests/second |
| `Parameters:MediumOperationsPermitLimit` | `50` requests/second |
| `Parameters:HeavyOperationsPermitLimit` | `20` requests/second |
| `Parameters:AdminOperationsPermitLimit` | `50` requests/minute |
| `Parameters:MaxConcurrentRequestsPerUser` | `20` (database protection) |
| `Parameters:MaxQueryComplexityPerWindow` | `1000` (query units per window) |
| `Parameters:BaseQueryComplexity` | `10` (units per query) |

### Environment Variable Support

All parameters support environment variable overrides using `__` (double underscore) as the hierarchy separator:
...
Parameters__MaxConcurrentRequestsPerUser=50
Parameters__MaxQueryComplexityPerWindow=5000
```
