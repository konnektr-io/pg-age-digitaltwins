...
- **TrackLastUpdatedBy**: Fully supported. The native SDK's custom JSON converters handle `$lastUpdatedBy` at both the twin-level (`$metadata.$lastUpdatedBy`) and per-property (`$metadata.<property>.lastUpdatedBy`) metadata. See [Security](/docs/graph/advanced-topics/security) for details.
