...
Welcome! This single guide combines "Quickstart" and "First Steps" to get you building on Konnektr Graph fast, with clear auth instructions and working samples.
...
```
https://explorer.graph.konnektr.io?x-adt-url=your-graph-name.api.graph.konnektr.io
```
...
- Grant type: client_credentials (M2M) or Authorization Code (user auth)
...
curl -s -X POST \
    ...
    https://your-graph-name.api.graph.konnektr.io/models
...
curl -s -X PUT \
    ...
    https://your-graph-name.api.graph.konnektr.io/digitaltwins/room-1
...
curl -s -X POST \
    ...
    https://your-graph-name.api.graph.konnektr.io/query
```
...
# 2) Upload DTDL model
models_url = "https://your-graph-name.api.graph.konnektr.io/models"
model_payload = [
        {
                "@id": "dtmi:com:sample:Room;1",
                "@type": "Interface",
...
# 3) Create twin
put_twin_url = "https://your-graph-name.api.graph.konnektr.io/digitaltwins/room-1"
twin_payload = {
        "$dtId": "room-1",
        "$metadata": { "$model": "dtmi:com:sample:Room;1" },
        "name": "Sample Room",
...
# 4) Query twins
query_url = "https://your-graph-name.api.graph.konnektr.io/query"
...
// 2) Upload DTDL model
await axios.post('https://your-graph-name.api.graph.konnektr.io/models', [
    {
        '@id': 'dtmi:com:sample:Room;1',
        '@type': 'Interface',
        '@context': ['dtmi:dtdl:context;3'],
...
// 3) Create twin
await axios.put('https://your-graph-name.api.graph.konnektr.io/digitaltwins/room-1', {
    '$dtId': 'room-1',
    '$metadata': { '$model': 'dtmi:com:sample:Room;1' },
    name: 'Sample Room',
    temperature: 21.5
...
// 4) Query twins
const { data } = await axios.post('https://your-graph-name.api.graph.konnektr.io/query', { query: 'SELECT * FROM digitaltwins' }, { headers });
console.log(data);
```
...
await http.PostAsJsonAsync("https://your-graph-name.api.graph.konnektr.io/models", modelPayload);
...
await http.PutAsJsonAsync("https://your-graph-name.api.graph.konnektr.io/digitaltwins/room-1", twinPayload);

// 4) Query twins
var queryPayload = new { query = "SELECT * FROM digitaltwins" };
var queryResp = await http.PostAsJsonAsync("https://your-graph-name.api.graph.konnektr.io/query", queryPayload);
...
try:
        ...
        import requests
        resp = requests.post("https://your-graph-name.api.graph.konnektr.io/query", json={"query": "SELECT * FROM models"}, headers=headers)
