...
Captured through PostgreSQL logical replication (durable and replicated):
 ...
 - **Model Events**: `ModelCreate`, `ModelUpdate`, `ModelDelete` (in progress)
...
### Webhooks
Push events to HTTP endpoints via POST requests. Supports Basic Auth, Bearer Token, and API Key headers. Ideal for:
- Serverless functions (Azure Functions, AWS Lambda)
- Logic Apps / Power Automate
- Custom integrations with legacy systems
- Notification systems (Slack, Teams via incoming webhooks)
...
AgeDigitalTwins supports flexible configuration for both hosted (Konnektr Cloud) and self-hosted environments.

### Hosted (Konnektr Cloud)

In the Konnektr Portal, you can configure event sinks via the UI:
1. Navigate to your Digital Twin instance.
2. Go to **Settings** > **Event Routing**.
3. Click **Add Sink** and choose the sink type (Kafka, Kusto, MQTT, Webhook).
4. Fill in the required details (connection strings, URLs, credentials).

*(Screenshots to be added)*

### Self-Hosted (Kubernetes / Docker)

For self-hosted instances, configure event routing via `appsettings.json` or environment variables.
...
webhook:
  - name: MyWebhookSink
    url: https://api.mysystem.com/events
    authenticationType: Basic  # None, Basic, Bearer, ApiKey
    username: my-user
