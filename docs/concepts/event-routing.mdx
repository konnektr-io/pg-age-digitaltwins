...
**Event Sinks** define *where* events should be sent (Kafka broker, MQTT broker, webhook endpoint, NATS server, Azure Data Explorer cluster, etc.). You configure connection details and authentication for each sink.
...
Archive events to PostgreSQL or TimescaleDB for time-series analytics, historical queries, and compliance using SQL you already run. History tables use snake_case names and native PostgreSQL types, and can live in the graph database itself or in a separate one.

**Common use cases:**
- Complete data history with temporal SQL queries
- Reconstruct graph state at any point in time
- Compliance and audit logging
- TimescaleDB compression, retention policies, and continuous aggregates

See [Data History with PostgreSQL / TimescaleDB](/docs/graph/guides/data-history-postgres) for detailed guide.
...
Publish events to [NATS](https://nats.io) — a high-performance cloud-native messaging system. Supports both core NATS pub/sub and JetStream for durable, at-least-once delivery.

**Common use cases:**
- Cloud-native microservices communication
- Event-driven architectures requiring high throughput and low latency
- Durable event streaming with JetStream
- Edge and IoT deployments with lightweight footprint

The NATS sink supports multiple authentication methods: token-based, username/password, and OAuth 2.0 (token exchange). Events can be published in CloudEvents binary content mode (default) or structured content mode.

See the [NATS Event Sink](/docs/graph/guides/event-sinks/nats) reference for full configuration details.
...
4. Fill in connection details:
   ...
   - **NATS**: Server URL, subject, authentication (token, username/password, or OAuth)
...
config:
  ...
  eventSinks:
    ...
    postgres:
      - name: PostgresDataHistory
        ...
        # Optional: schema and table name overrides (defaults: public / adt_*_events)
        # schema: public
        # propertyEventsTable: adt_property_events
    ...
    nats:
      - name: NatsSink1
        ...
        useBinaryMode: true
        # Optional: username/password or token auth
        # username:
        #   valueFrom:
        #     secretKeyRef:
  ...
  eventRoutes:
    ...
    - sinkName: PostgresDataHistory
      eventFormat: DataHistory        # SQL-queryable history
...
See [Data History with Azure Data Explorer](/docs/graph/guides/data-history-kusto) for complete guide with query examples, or [Data History with PostgreSQL / TimescaleDB](/docs/graph/guides/data-history-postgres) when your history lives in PostgreSQL.
...
- **[Data History with PostgreSQL / TimescaleDB](../how-to-guides/data-history-postgres)** - SQL-queryable history with hypertables
