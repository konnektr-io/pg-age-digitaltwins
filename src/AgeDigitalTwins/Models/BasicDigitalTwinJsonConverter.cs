...
// Based on Azure.DigitalTwins.Core.BasicDigitalTwinJsonConverter (MIT License)
...
{
    /// <inheritdoc/>
    public override BasicDigitalTwin Read(
    ...
    )
    {
        ...
        {
            throw new JsonException("Cannot deserialize null as BasicDigitalTwin");
        }
        ...
        while (reader.TokenType != JsonTokenType.EndObject)
        {
            ...
            {
                twin.Id = reader.GetString() ?? string.Empty;
            }
            ...
            else if (propertyName == DigitalTwinsJsonPropertyNames.DigitalTwinMetadata)
            {
                ...
                if (
                    metadataBlock.TryGetProperty(
                        DigitalTwinsJsonPropertyNames.MetadataLastUpdateTime,
                        out JsonElement lastUpdateTime
                    )
                )
                {
                    twin.LastUpdatedOn = lastUpdateTime.TryGetDateTimeOffset(
                ...
                var metadata = JsonSerializer.Deserialize<DigitalTwinMetadata>(
                    metadataBlock.GetRawText(),
                    options
                );
                if (metadata != null)
                {
            ...
            {
                twin.Contents[propertyName] = JsonSerializer.Deserialize<JsonElement>(
                    ref reader,
                    options
                );
            }
    ...
    )
    {
        ...
        writer.WriteString(DigitalTwinsJsonPropertyNames.DigitalTwinETag, value.ETag);
        ...
        string partialMetadata = JsonSerializer.Serialize<DigitalTwinMetadata>(
            value.Metadata,
            options
        );
        ...
        {
            throw new NotSupportedException("Metadata must be a JSON object");
        }
