...
public class WebhookEventSink : IEventSink, IDisposable
{
    ...
    private void ConfigureClient()
    {
        ...
        else if (_options.AuthenticationType.Equals("Bearer", StringComparison.OrdinalIgnoreCase))
        {
            ...
            _options.AuthenticationType.Equals("ApiKey", StringComparison.OrdinalIgnoreCase)
            || _options.AuthenticationType.Equals("Header", StringComparison.OrdinalIgnoreCase)
        )
        {
            if (
                !string.IsNullOrEmpty(_options.HeaderName)
                && !string.IsNullOrEmpty(_options.HeaderValue)
            )
            {
    ...
    )
    {
        ...
        if (useOAuth && _credential != null)
        {
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Failed to retrieve OAuth token for Webhook sink '{SinkName}'",
                    Name
                );
                throw;
            }
        ...
        foreach (var cloudEvent in cloudEvents)
        {
            try
            {
                ...
                if (useOAuth && oauthToken != null)
                {
                    ...
                    if (!response.IsSuccessStatusCode)
                    {
                        ...
                        _logger.LogError(
                            "Webhook sink '{SinkName}' failed to send event {EventId}. Status: {StatusCode}, Response: {Response}",
                            Name,
                            cloudEvent.Id,
                            response.StatusCode,
                            responseBody
                    ...
                    else
                    {
                        _isHealthy = true;
                        _logger.LogDebug(
                            "Webhook sink '{SinkName}' successfully sent event {EventId}",
                            Name,
                            cloudEvent.Id
                        );
                    }
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(
                    ex,
                    "Error sending event {EventId} to webhook sink '{SinkName}'",
                    cloudEvent.Id,
                    Name
                );
                throw;
